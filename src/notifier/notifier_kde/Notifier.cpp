    ...
    QObject( parent )
{
    ...
    m_tray->setTitle( QString( tr ( "Condres Settings Manager" ) ) );
    m_tray->setIconByName( "condres-settings-manager" );
    m_tray->setStatus( KStatusNotifierItem::Passive );

    auto menu = m_tray->contextMenu();

    QAction* msmKernel = new QAction( QIcon( ":/icons/tux-condres.png" ),
                                      QString( tr ( "Kernels" ) ),
                                      menu );
    ...
    {
        QProcess::startDetached( "condres-settings-manager", QStringList() << "-m" << "msm_kernel" );
        m_tray->setStatus( KStatusNotifierItem::Passive );
    } );
    connect( msmLanguagePackages, &QAction::triggered, this, [msmLanguagePackages, this]()
    {
        QProcess::startDetached( "condres-settings-manager", QStringList() << "-m" << "msm_language_packages" );
        m_tray->setStatus( KStatusNotifierItem::Passive );
    } );
...
Notifier::cLanguagePackage()
{
    ...
    if ( !packages.isEmpty() )
    {
        ...
        m_tray->showMessage( tr( "Condres Settings Manager" ),
...
Notifier::cKernel()
{
    ...
    if ( m_checkUnsupportedKernel && !unsupportedKernels.isEmpty() )
    {
        ...
        {
            m_tray->setStatus( KStatusNotifierItem::Active );
            m_tray->showMessage( QString( tr( "Condres Settings Manager" ) ),
...
{
    QSettings settings( "condres", "condres-settings-manager" );
...
{
    QSettings settings( "condres", "condres-settings-manager-Notifier" );
